...
import Project.Client.Interfaces.IAwarenessEvent;
...
public enum Client {
    ...
    private boolean currentCooldownEnabled = false;
    ...
    /**
     * Sends a Rock-Paper-Scissors pick to the server
     * 
     * @param choice "r", "p", or "s"
     * @throws IOException
     */
    public void sendPick(String choice) throws IOException {
        Payload payload = new Payload();
        payload.setMessage(choice);
        payload.setPayloadType(PayloadType.PLAYER_PICK);
        sendToServer(payload);
    }

    /**
     * Sends away status to server
     * 
     * @param isAway true if marking away
     * @throws IOException
     */
    public void sendAway(boolean isAway) throws IOException {
        AwayPayload ap = new AwayPayload();
        ap.setAway(isAway);
        // server should set clientId authoritatively, but include local id for
        // convenience
        ap.setClientId(myUser.getClientId());
        sendToServer(ap);
    }

    /**
     * Sends the game mode selection (RPS-3, RPS-5, etc) to the server
     * 
     * @param gameMode
     * @throws IOException
     */
    public void sendGameMode(GameMode gameMode, boolean cooldownEnabled) throws IOException {
    ...
    private void processPayload(Payload payload) {
        switch (payload.getPayloadType()) {
            ...
            case PayloadType.PHASE:
                ...
                break;
            case PayloadType.GAME_MODE:
                processGameMode(payload);
                break;
            case PayloadType.GAME_OVER:
            ...
            case PayloadType.POINTS:
                ...
                break;
            case PayloadType.ELIMINATION:
                processElimination(payload);
                break;
            default:
    ...
    private void processElimination(Payload payload) {
        if (!(payload instanceof Project.Common.EliminationPayload)) {
            error("Invalid payload subclass for processElimination");
            return;
        }
        Project.Common.EliminationPayload ep = (Project.Common.EliminationPayload) payload;
        long targetId = ep.getClientId();
        boolean eliminated = ep.isEliminated();

        if (knownClients.containsKey(targetId)) {
            knownClients.get(targetId).setEliminated(eliminated);
            passToUICallback(Project.Client.Interfaces.IEliminationEvent.class,
                    e -> e.onPlayerEliminated(targetId, eliminated));
        }
    }
    ...
    private void processGameMode(Payload payload) {
        if (!(payload instanceof GameModePayload)) {
            LoggerUtil.INSTANCE.warning("Received non-GameModePayload for GAME_MODE type");
            return;
        }
        GameModePayload gmp = (GameModePayload) payload;
        currentGameMode = gmp.getGameMode();
        currentCooldownEnabled = gmp.isCooldownEnabled();
        System.out.println(TextFX.colorize(
                "Game mode changed to: " + currentGameMode.name() + " cooldown=" + currentCooldownEnabled,
                Color.YELLOW));

        passToUICallback(IGameModeEvent.class, e -> e.onGameModeChange(currentGameMode, currentCooldownEnabled));
    }

    private void processAway(Payload payload) {
        if (!(payload instanceof AwayPayload)) {
            LoggerUtil.INSTANCE.warning("Received non-AwayPayload for AWAY type");
            return;
        }
        AwayPayload ap = (AwayPayload) payload;
        long clientId = ap.getClientId();
        boolean isAway = ap.isAway();
        // Update local known client if present
        if (knownClients.containsKey(clientId)) {
            knownClients.get(clientId).setAway(isAway);
        }

        // Broadcast to UI: update user list visual
        passToUICallback(IAwarenessEvent.class, e -> e.onAwayStatusChanged(clientId, isAway));

        // Also notify game events panel
        String display = getDisplayNameFromId(clientId);
        String msg = String.format("%s is %s", display, isAway ? "away" : "no longer away");
        passToUICallback(IMessageEvents.class, e -> e.onMessageReceive(Constants.GAME_EVENT_CHANNEL, msg));
    }

    private void processGameOver(Payload payload) {
        if (!(payload instanceof GameOverPayload)) {
            LoggerUtil.INSTANCE.warning("Received non-GameOverPayload for GAME_OVER type");
            return;
        }
        GameOverPayload gp = (GameOverPayload) payload;
        String message = gp.getMessage();
        if (message == null || message.isEmpty()) {
            if (gp.getWinners() == null || gp.getWinners().isEmpty()) {
                message = "Game over: no winners.";
            } else if (gp.getWinners().size() == 1) {
                message = String.format("%s wins!", gp.getWinners().get(0));
            } else {
                message = String.format("Tie between: %s", String.join(", ", gp.getWinners()));
            }
        }

        // Simply log the game over message to the event log (no popup)
        final String finalMessage = message;
        passToUICallback(IMessageEvents.class, e -> e.onMessageReceive(Constants.GAME_EVENT_CHANNEL, finalMessage));
    }
