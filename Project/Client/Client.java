...
// bs768, 11/24/2025, Client class handling client-side logic and server communication
public enum Client {
    INSTANCE;
        ...
        if (text.startsWith(Constants.COMMAND_TRIGGER)) {
            ...
            if (isConnection("/" + text)) {
                ...
                knownClients.forEach((key, value) -> {
                    LoggerUtil.INSTANCE.info(TextFX.colorize(String.format("%s%s Points: %d", value.getDisplayName(),
                            key == myUser.getClientId() ? " (you)" : "", value.getPoints()), Color.CYAN));
                });
                wasCommand = true;
    ...
    private void sendReady() throws IOException {
        Payload payload = new Payload();
        payload.setPayloadType(PayloadType.READY_CHECK);
        sendToServer(payload);
    }
    ...
    private void processPointsUpdate(Payload payload) {
        if (payload instanceof PointsPayload) {
            ...
            if (knownClients.containsKey(clientId)) {
                ...
                // LoggerUtil.INSTANCE.info(TextFX.colorize(String.format("Points updated for
                // %s: %d", knownClients.get(clientId).getDisplayName(), points), Color.GREEN));
            }
            if (myUser.getClientId() == clientId) {
                myUser.setPoints(points);
            }
        }
    }
    ...
    private void processRoomsList(Payload payload) {
        ...
        if (rooms == null || rooms.size() == 0) {
            LoggerUtil.INSTANCE.warning(TextFX.colorize("No rooms found matching your query", Color.RED));
            return;
        }
